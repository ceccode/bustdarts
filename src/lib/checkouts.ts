export const IMPOSSIBLE_TOTALS: Set<number> = new Set([163, 166, 169, 172, 173, 175, 176, 178, 179]);

export const CHECKOUTS: Record<number, string> = {
  170: 'T20·T20·Bull', 167: 'T20·T19·Bull', 164: 'T20·T18·Bull', 161: 'T20·T17·Bull',
  160: 'T20·T20·D20', 158: 'T20·T20·D19', 157: 'T20·T19·D20', 156: 'T20·T20·D18',
  155: 'T20·T19·D19', 154: 'T20·T18·D20', 153: 'T20·T19·D18', 152: 'T20·T20·D16',
  151: 'T20·T17·D20', 150: 'T20·T18·D18', 149: 'T20·T19·D16', 148: 'T20·T16·D20',
  147: 'T20·T17·D18', 146: 'T20·T18·D16', 145: 'T20·T19·D14', 144: 'T20·T20·D12',
  143: 'T20·T17·D16', 142: 'T20·T14·D20', 141: 'T20·T19·D12', 140: 'T20·T20·D10',
  139: 'T19·T14·D20', 138: 'T20·T18·D12', 137: 'T20·T19·D10', 136: 'T20·T20·D8',
  135: 'Bull·T15·D20', 134: 'T20·T14·D16', 133: 'T20·T19·D8',  132: 'Bull·T14·D20',
  131: 'T20·T13·D16', 130: 'T20·T20·D5',  129: 'T19·T16·D12', 128: 'T18·T14·D16',
  127: 'T20·T17·D8',  126: 'T19·T19·D6',  125: 'Bull·T17·D12', 124: 'T20·T16·D8',
  123: 'T19·T16·D9',  122: 'T18·T18·D7',  121: 'T20·T11·D14', 120: 'T20·S20·D20',
  119: 'T19·T12·D13', 118: 'T20·S18·D20', 117: 'T20·S17·D20', 116: 'T20·S16·D20',
  115: 'T20·S15·D20', 114: 'T20·S14·D20', 113: 'T20·S13·D20', 112: 'T20·T12·D20',
  111: 'T20·S11·D20', 110: 'T20·S10·D20', 109: 'T20·S9·D20',  108: 'T20·S8·D20',
  107: 'T19·S10·D20', 106: 'T20·S6·D20',  105: 'T20·S5·D20',  104: 'T18·S18·D16',
  103: 'T19·S6·D20',  102: 'T20·S10·D16', 101: 'T17·Bull',     100: 'T20·D20',
  99:  'T19·S10·D16', 98:  'T20·D19',      97:  'T19·D20',     96:  'T20·D18',
  95:  'T19·D19',     94:  'T18·D20',      93:  'T19·D18',     92:  'T20·D16',
  91:  'T17·D20',     90:  'T20·D15',      89:  'T19·D16',     88:  'T20·D14',
  87:  'T17·D18',     86:  'T18·D16',      85:  'T15·D20',     84:  'T20·D12',
  83:  'T17·D16',     82:  'Bull·D16',     81:  'T19·D12',     80:  'T20·D10',
  79:  'T13·D20',     78:  'T18·D12',      77:  'T19·D10',     76:  'T20·D8',
  75:  'T17·D12',     74:  'T14·D16',      73:  'T19·D8',      72:  'T16·D12',
  71:  'T13·D16',     70:  'T18·D8',       69:  'T19·D6',      68:  'T20·D4',
  67:  'T17·D8',      66:  'T10·D18',      65:  'T19·D4',      64:  'T16·D8',
  63:  'T13·D12',     62:  'T10·D16',      61:  'T15·D8',      60:  'S20·D20',
  58:  'S18·D20',     56:  'S16·D20',      54:  'S14·D20',     52:  'S12·D20',
  50:  'Bull',        48:  'S16·D16',      46:  'S6·D20',       44:  'S12·D16',
  42:  'S10·D16',     40:  'D20',          38:  'D19',          36:  'D18',
  34:  'D17',         32:  'D16',          30:  'D15',          28:  'D14',
  26:  'D13',         24:  'D12',          22:  'D11',          20:  'D10',
  18:  'D9',          16:  'D8',           14:  'D7',           12:  'D6',
  10:  'D5',          8:   'D4',           6:   'D3',           4:   'D2',
  2:   'D1',
};

export function checkoutFor(score: number, doubleOut: boolean): string | null {
  if (!doubleOut) return null;
  if (score < 2 || score > 170) return null;
  if (IMPOSSIBLE_TOTALS.has(score)) return null;
  return CHECKOUTS[score] ?? null;
}
